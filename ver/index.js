...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=558183064666 :+55 81 83064666\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' porfavor escaneia essa desgraca '))
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*Aguarde... Obtendo Conexão IP.*',
				success: '✔️ Sucesso ✔️',
                                levelon: '*Nivel Habilitado*',
				leveloff: ' *Nivel Desabilitado!*',
				levelnoton: 'level não foi ativo*',
				levelnol: '*Pqp kskst level* 0 ',
				error: {
					stick: '[❗] Falha, porfavor, tente novamente. Pode ter Acontecido pelo formato da figurinha❌',
					Iv: '❌ Link inválido ❌'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos! ❌',
					ownerG: '[❗] Este comando só pode ser usado pelo grupo proprietário! ❌',
					ownerB: '[❗] Este comando só pode ser usado pelo Vinicius ou Premiuns! ❌',
					admin: '[❗] Este comando só pode ser usado por administradores de grupo! ❌',
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE  | ViniciusBOT WA DEV BOT」 ──\nOlá como!\nVocê não está registrado, \n\nComando : ${prefix}daftar nome\nExemplo : ${prefix}daftar vinicius`,
				}
			}
			...
			const ownerNumber = ["558183064666@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '558183064666@s.whatsapp.net'
                        /******ApiKey Input******/
            ...
            try {
                ...
                if (requiredXp <= getLevelingXp(sender)) {
                    ...
                    await reply(`*「 Aumentou de LVL! - ViniciusBOT 」*\n\n➸ *XP*: ${getLevelingXp(sender)}\n➸ *Nivel*: ${getLevel} -> ${getLevelingLevel(sender)}`)
                }
            } catch (err) {
                console.error(err)
            }
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `perdeu adm mano, chorou :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`Perdeu adm mano, chorou @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Aeee mano, agr virou adm! : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
	...
	case 'hidetag':
              ...
              await client.sendMessage(from, options, text)
               break
					...
					try {
						...
						reply('username tiktok invalido, amigão facilita ai')
					}
					break
				case 'snowwrite':
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'testime':
					setTimeout( () => {
					client.sendMessage(from, '*SE EXPLODA AGORA*', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
	...
	case 'kbbi':
               ...
               if (data.error) return reply(data.error)
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista de ademir aq*${groupMetadata.subject}*\n\nTotal : ${groupAdmins.length}\n\n`
					no = 0
		...
		if (args.length < 1) return reply ('*PARCEIROS*: \nEaxy: https://youtube.com/c/EAXYFF\nYamii: https://youtube.com/c/TioYamiiroivemsmpaki\nSaad: https://youtube.com/channel/UCtX3fzhC-teLUpaS7Xz8nKA\nKazuma: https://youtube.com/channel/UChRgBlar0IKGAsseuBeNRgA')
				...
				case 'block':
					...
					client.sendMessage(from, `Pedidos recebidos, bloquear ${body.slice(8)}`, text)
					break
				case 'hilih':
					client.updatePresence(from, Presence.composing) 
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 Vinicius BOT 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘  BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 TVinicius BOT 〙', text, {quoted: mek})
					break
                case 'tagall5':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply('escreve desgraça')
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 TRANSMISSÃO AUTO 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 TRANSMISSÃO AUTO 」*\n\n${body.slice(4)}`)
						}
					...
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply('voce poderia por gentileza escrever?.')
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 MENSAGEM DE GRUPO. 」*\n*Grupo* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `*「 MENSAGEM URGENTE. 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
						}
           ...
           break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`É BAN ADM?? TÁ CERTO: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
			...
			case 'closegc':
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                   ...
                   break
					...
					break
                                  case 'cadastrar':
									  case 'daftar' :
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('tais registrado fdp')
					if (args.length < 1) return reply(`escreveu errado corno \nComando: ${prefix}cadastrar nome|idade\nContoh : ${prefix}cadastrar Vinicius`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./database/json/user.json', JSON.stringify(user))
						client.sendMessage(from, `O registro foi bem sucedido, total de usuários ${user.length}\``, text, {quoted: mek})
					break
                                case 'bemvindo':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
                ...
                resul = `◪ *Perfil*\n  ├─ *Nome* : ${sem}\n \n Conta: Usuario Simples\n \n Função: Usuario Simples \n *EXP* : ${userXp}\n  *Level* : ${userLevel}`
					...
					case 'nsfwarmpits':
						...
						break
			   	        ...
			   	        if (!isUser) return reply(mess.only.userB)
                                        ...
                                        client.updatePresence(from, Presence.composing) 
				        uptime = process.uptime()
                                        client.sendMessage(from, `Velocidade do *ViniciusBOT*: *${latensi.toFixed(4)} _Segundos_*`, text, { quoted: mek})
                                        break
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
